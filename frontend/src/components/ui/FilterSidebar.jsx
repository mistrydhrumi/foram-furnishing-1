...
const resetFilters = () => {
  ...
  return (
    <div className="bg-gray-100 mt-10 p-4 rounded-md h-max hidden md:block w-64">
      ...
      <div className="flex flex-col gap-2 mt-3">
        {UniqueCategory.map((item, index) => (
          <div key={index} className="flex items-center gap-2">
            <input type="radio" checked={category === item} onChange={()=>handleCategoryClick(item)} name="category" id={`cat-${index}`} />
            <label htmlFor={`cat-${index}`} className="cursor-pointer">
              {item}
            </label>
          </div>
        ))}
      ...
      {/* Reset Button*/ }
      <Button onClick={resetFilters} className="bg-blue-600 text-white mt-5 cursor-pointer w-full">Reset Button</Button>
    </div>
  );
};
